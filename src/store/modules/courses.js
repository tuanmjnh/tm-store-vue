...
export default {
  ...
  getters: {
    ...
    getById: state => id => {
      return state.data.find(data => data.course_id === id)
    },
  ...
  actions: {
    ...
    async update({ commit }, data) {
      await axios.mle
        .put(controller + data.course_id, data)
        .then(function(res) {
    ...
    async delete({ commit, state }, data) {
      var _data = []
      data.forEach(i => { _data.push({ course_id: i.selected.course_id, status: i.status }) });
      await axios.mle
