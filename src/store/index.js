      ...
      for (let index = 0; index < key.length; index++) {
        ...
        if (e.length < 2) {
          ...
          if (!state.$dictionary[e0[0]]) rs += key[index];
            ...
            rs +=
              index > 0
                ? state.$dictionary[e0[0]].toLowerCaseFirst()
                : state.$dictionary[e0[0]];
          if (e0.length > 1) rs = rs.format(e0[1]);
        } else if (e.length < 3) {
          const e1 = e[1].split(':');
          if (!state.$dictionary[e[0]] || !state.$dictionary[e[0]][e1[0]])
            rs += key[index];
          else
            rs +=
              index > 0
                ? state.$dictionary[e[0]][e1[0]].toLowerCaseFirst()
                : state.$dictionary[e[0]][e1[0]];
  ...
  mutations: {
    ...
    SET_CATCH(state, error) {
      if (error.response.status === 401) {
        ...
        state.$message.text =
          state.$dictionary.auth && state.$dictionary.auth.msg_err_expired
            ? state.$dictionary.auth.msg_err_expired
            : error.response.statusText;
      } else {
        // error.response
        state.$message.text =
          state.$dictionary.error && state.$dictionary.error.connection
            ? state.$dictionary.error.connection
            : error.message;
        console.log(error);
      }
